...
import pandas as pd
import csv
from scipy.fftpack import fftn, ifftn, dct, idct # n次元離散フーリエ・逆フーリエ変換，離散コサイン変換
...
for p in range(iteration_num):
    ...
    grad_w = (-1)*(1/N)*dct(L.T@(y-L@temp_trans))              #勾配計算 #変換#
    MMw = w_fista - grad_w*(1/Lip)                              #更新点候補
    temp_trans = idct(MMw)                                     #逆変換#
...
plot_y2 = idct(g2)     #逆変換で最終的に得たいデータに変換#
plt.figure(3)
plt.title("x_fista")
