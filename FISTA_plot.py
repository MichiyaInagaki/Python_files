...
for p in range(100):
    ...
    kbata = bata                                                #前回のβ
    #three_w_fista = np.reshape(w_fista,(Zrange,Yrange,Xrange))  #3次元配列版x^^^
    #z_fista = ifftn(three_w_fista)                              #3次元逆フーリエ変換^^^
    #z_fista = np.reshape(z_fista,(cell_size,1))                 #1次元に直す^^^
    z_fista = ifftn(w_fista)    #^^^
    ...
    grad_x = (-1)*(1/N)*fftn(L.T@(y-L@z_fista))                 #勾配計算 ***
    MMx = w_fista - grad_x*(1/Lip)                              #更新点候補 ***
    ...
    while MMtemp < temp:
        ...
        MMx = w_fista - grad_x*(1/Lip)                           #更新候補位置再計算
        #three_MMx = np.reshape(MMx,(Zrange,Yrange,Xrange))      #3次元配列版MMx^^^
        #MMz = ifftn(three_MMx)                                  #3次元逆フーリエ変換^^^
        #MMz = np.reshape(MMz,(cell_size,1))                     #1次元に直す^^^
        MMz = ifftn(MMx)
    ...
    w_fista = x_ista + ((kbata-1)/bata)*(x_ista-kx_ista)        #高速化部分　wの更新
    print("step:",p,"Absolute error:",np.linalg.norm(x_ista-kx_ista,ord=2),"Relative error:",(np.linalg.norm(x_ista-kx_ista,ord=2)/np.linalg.norm(kx_ista, ord=2)))
    #minの更新
    #if min > np.linalg.norm(x_ista-kx_ista,ord=2)/np.linalg.norm(kx_ista, ord=2) and p>3:
        #min = np.linalg.norm(x_ista-kx_ista,ord=2)/np.linalg.norm(kx_ista, ord=2)   
    #相対誤差が大きいほうに転じたとき反復を終了する
    #if min+0.003 < np.linalg.norm(x_ista-kx_ista,ord=2)/np.linalg.norm(kx_ista, ord=2):
        #break   
...
g2[(g_abs) < sort_x[int(cell_size/100*70)]] = 0 #ノイズを切る 75%スパース
plt.figure(2)
...
cut_concentration = sort_concentration_list[int(cell_size/100*96)]
